...
export default {
    ...
    addTask: (project, task) => {
        db.collection('users').doc(project.userId).collection('projects').doc(project.id).collection('tasks').add(task, {merge:true})
    },
    ...
    getTasks: async project => {
        const projectDoc = await db.collection('users').doc(project.userId).collection('projects').doc(project.id).collection('tasks').get()
        const tasks = projectDoc.docs.map(doc => {
